...
The anomaly threshold (`mean + threshold_std_multiplier * std` of normal-image
distances) was originally set at a 3.0 std multiplier, which was precise
(0.98 precision) but missed 14 of 63 defective test images (0.78 recall).
`tune_threshold.py` swept multipliers from 1.5 to 3.0 against the labeled
bottle test set and found 1.5 gives the best F1: recall rises to 0.92 (only
5 false negatives) while precision barely moves (0.983, one additional false
